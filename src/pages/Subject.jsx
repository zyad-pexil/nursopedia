...
// Enhanced re-auth dialog (UI only, same logic)
function ReAuth({ onVerify, loading }) {
  ...
  return (
    <div className="min-h-[60vh] grid place-items-center bg-gradient-to-b from-emerald-50/40 to-transparent rounded-xl">
      <Card className="w-full max-w-md border-0 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.2)] ring-1 ring-black/5">
        <CardHeader className="text-center">
          <div className="mx-auto mb-2 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-600 text-white shadow-sm">
            {/* lock icon */}
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-6 w-6">
              <path d="M17 8h-1V6a4 4 0 10-8 0v2H7a2 2 0 00-2 2v8a2 2 0 002 2h10a2 2 0 002-2v-8a2 2 0 00-2-2zm-6 8h2v2h-2v-2zm3-8H10V6a2 2 0 114 0v2z" />
            </svg>
          </div>
          <CardTitle className="text-xl font-extrabold tracking-tight">إعادة التحقق</CardTitle>
          <CardDescription className="text-sm">لأمانك، يرجى إعادة إدخال اسم المستخدم وكلمة المرور</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleVerify} className="space-y-4">
            {error && (
              <div role="alert" className="flex items-start gap-2 rounded-md border border-red-200 bg-red-50 p-2 text-sm text-red-700">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="mt-0.5 h-4 w-4"><path d="M11 7h2v6h-2V7zm0 8h2v2h-2v-2z"/></svg>
                <span>{error}</span>
              </div>
            )}

            <div className="space-y-1.5">
              <Label htmlFor="reauth-username">اسم المستخدم</Label>
              <div className="relative">
                ...
                <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-gray-400">
                  {/* user icon */}
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4"><path d="M12 12a5 5 0 100-10 5 5 0 000 10zm0 2c-4.418 0-8 2.239-8 5v1h16v-1c0-2.761-3.582-5-8-5z"/></svg>
                </span>
              </div>
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="reauth-password">كلمة المرور</Label>
              <div className="relative">
                <Input
                  id="reauth-password"
                  name="password"
                ...
                >
                  {/* eye icon */}
                  {showPassword ? (
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4"><path d="M12 6c-5 0-9 4-10 6 1 2 5 6 10 6s9-4 10-6c-1-2-5-6-10-6zm0 10a4 4 0 110-8 4 4 0 010 8z"/></svg>
                  ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4"><path d="M12 6c-5 0-9 4-10 6 .54 1.08 1.59 2.42 3.03 3.6L3 18l1.41 1.41 16.97-16.97L20.97 1 19.56 2.41 17.62 4.35C15.99 3.76 14.08 3.5 12 3.5c-5 0-9 4-10 6 1.02 2.03 3.57 4.74 7 5.68l-1.7 1.7C4.76 15.93 2.83 14.01 2 12c1-2 5-6 10-6 1.35 0 2.63.23 3.82.66l-1.6 1.6A8.8 8.8 0 0012 8c-4.42 0-8 2.24-9 4 0 .52 1.35 2.04 3.56 3.26l-1.42 1.42C2.59 15.42 1.54 14.08 1 13c1-2 5-6 11-6 .88 0 1.74.09 2.56.27l-1.45 1.45A4 4 0 0012 8a4 4 0 014 4c0 .5-.1.98-.27 1.42l-1.47 1.47A5.98 5.98 0 0012 16a5.96 5.96 0 01-3.4-1.04l-1.45 1.45A7.95 7.95 0 0012 18c5 0 9-4 10-6-.39-.78-1.06-1.67-1.92-2.54l-1.41 1.41c.73.6 1.33 1.27 1.73 1.88-1 1.76-4.58 4-9 4-1.23 0-2.41-.2-3.5-.57l-1.34 1.34C8.64 17.83 10.27 18 12 18c5 0 9-4 10-6-.53-1.06-1.56-2.39-2.97-3.55L17.65 6.8A9.77 9.77 0 0012 6z"/></svg>
                  )}
                </button>
              </div>
            </div>
            ...
            <Button type="submit" disabled={loading} className="w-full bg-emerald-600 text-white hover:bg-emerald-700">
              {loading ? (
                <span className="inline-flex items-center gap-2">
                  <span className="h-3 w-3 animate-spin rounded-full border-2 border-white/60 border-t-transparent"></span>
                  جاري التأكيد...
                </span>
              ) : 'تأكيد'}
            </Button>

            <p className="text-center text-xs text-gray-500">لن يتم حفظ كلمة المرور. هذا الإجراء لحماية حسابك فقط.</p>
          </form>
        </CardContent>
      </Card>
    </div>
...
export default function SubjectPage() {
  ...
  return (
    <div className="p-0 md:p-0 nurso-section">
      {!verified ? (
        <ReAuth onVerify={() => setVerified(true)} loading={loading} />
      ) : (
